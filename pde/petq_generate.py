...
def petq_generate():
    ...
    regions_quad = entities[numpy.argwhere(entities[:,0]==3)[:,0]][:,1]
    ...
    #         to = (elemNodeTags[i]-1).reshape(int(elemNodeTags[i].shape[0]/3),3,order = 'C')
    #     if elemTypes[i] == 3: # 4-node quadrangle (see comment at top)
    #         qo = (elemNodeTags[i]-1).reshape(int(elemNodeTags[i].shape[0]/4),4,order = 'C')
            
                
    for k in range(regions_trig.size):
        elemTypes, elemTags, elemNodeTags = gmsh.model.mesh.getElements(dim = 2,tag = regions_trig[k])
        new_t = (elemNodeTags[0]-1).reshape(int(elemNodeTags[0].shape[0]/3),3,order = 'C')
        reg_t = int(regions_trig[k])*numpy.ones(new_t.shape[0],dtype = numpy.uint64)
        t = numpy.append(t,numpy.c_[new_t,reg_t],axis = 0)
        
    for k in range(regions_quad.size): # check this!
