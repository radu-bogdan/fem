...
def do():
    ...
    for i in range(iterations):
        ...
        for j in range(int(T/dt)):
            ...
            s1 = cuda_iMh_K.dot(cuda_uh_NC1_old)
            s2 = cuda_iMh.dot(cuda_intF)
            s3 = cuda_iMh_Mh_sigma.dot((cuda_uh_NC1_old-cuda_uh_NC1_oldold)/dt)
            
            # cuda_uh_NC1 = 2*cuda_uh_NC1_old-cuda_uh_NC1_oldold-(dt**2)*(cuda_iMh_K@cuda_uh_NC1_old +cuda_iMh@cuda_intF +cuda_iMh_Mh_sigma@(cuda_uh_NC1_old-cuda_uh_NC1_oldold)/dt)
            
            # ok = (cuda_iMh_K.dot(cuda_uh_NC1_old) +cuda_iMh.dot(cuda_intF) +cuda_iMh_Mh_sigma.dot(cuda_uh_NC1_old-cuda_uh_NC1_oldold)/dt)
            cuda_uh_NC1 = 2*cuda_uh_NC1_old-cuda_uh_NC1_oldold-(dt**2)*(s1+s2+s3)
