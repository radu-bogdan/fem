...
export mysimplexquad

"""
    mysimplexquad([::Type{T},] N::Int, n::Int)
    mysimplexquad(N::Int, vert::AbstractMatrix)
...
# Sample Usage:
    ...
    X, W = mysimplexquad(n, vert)     # Specify the vertices
    X, W = mysimplexquad(T, n, dim)   # Specify the dimension and use unit simplex
...
function mysimplexquad(::Type{T}, N::Int, n::Int) where {T}
    m = n + 1
    vert = diagm(m, n, ones(T, n))
    mysimplexquad(N, vert)
end
mysimplexquad(N::Int, n::Int) = mysimplexquad(float(Int), N, n)

function mysimplexquad(N::Int, vert::AbstractMatrix{T}) where {T}
