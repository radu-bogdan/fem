...
# Linear case with no current, refinements:
# Torque =  -2.9905489091572193e-05
...
# Torque =  36.36041633280734
# Torque =  35.749277625852415
# Torque =  35.462658895382354
...
def SolveNewton(a, rhs, maxit=100, maxerr=1e-11, dampfactor=1, printing=False, callback=None, linesearch=False, printenergy=False, print_wrong_direction=False, shift=0):
    ...
    for it in range(maxit):
        ...
        if a.condense:
            ...
            #solveLinearSystem(a, r, x=w, maxsteps = 10000, tol = 1e-12, restart = 200)
            solveLinearSystem(a, r, w, shift=shift)
...
def solveStateEquation(shift=0):
    if bLinear:
        rhs = LinearForm(fes)
        ...
        a.Assemble()
        solveLinearSystem(a, rhs.vec, gfu.vec, shift=shift)
    else:
        #SolveNewton(a, rhs, dampfactor=1.0, printing=False, maxit=200, maxerr=1e-8, shift=shift)
        a = BilinearForm(fes)
        a += Equation1a(u, v)
        a += Equation1b(u, v)
        a += Equation2(u, v)
        ...
        NewtonMinimization(a, gfu, freedofs=fes.FreeDofs(), maxit=100, maxerr=1e-11, inverse="sparsecholesky", dampfactor=0.05, linesearch=True, printing=True, callback=None)
        #NewtonMinimization(a, gfu, freedofs=fes.FreeDofs(), maxit=100, maxerr=1e-11, inverse="umfpack", dampfactor=0.05, linesearch=False, printing=False, callback=None)

def solveLinearSystem(a, rhs, x, shift=0):
    ...
    #sol[freedofsvec] = scspla.cg(aKMat[freedofsvec,:] [:,freedofsvec],rhs.FV().NumPy()[freedofsvec])
...
def incorpPeriodicity(array_corresponding, rows, cols, vals, rhsvec, symmetrize=False, shift=0):
    ells_per_row = defaultdict(list)
    for ell in range(len(vals)):
        r = rows[ell]
        ells_per_row[r].append(ell)

    L = array_corresponding.shape[0]
    for k in range(L):
        i = array_corresponding[k][0]
        j = array_corresponding[(k-shift)%L][1]
        for ell in ells_per_row[i]: #i-th row: 1*ui -1*uj = 0
            if cols[ell] == i:
                vals[ell] = 1.
            elif cols[ell] == j:
                vals[ell] = -1.
    ...
    ## Slow version
    #for k in range(array_corresponding.shape[0]):
        #i = array_corresponding[k][0]
        #j = array_corresponding[k][1]
        ##print(k, i, j)
        #for ell in range(len(vals)):
            #if rows[ell] == i:          #i-th row: 1*ui -1*uj = 0
                #if cols[ell] == i:
                    #vals[ell] = 1.
                #elif cols[ell] == j:
                    #vals[ell] = -1.
...
def identifyPointsAirgap():
    ...
    sliding_inner_reduced_points_idces = []
    sliding_inner_points_idces = []

    for k in range(N):
        p = mesh.ngmesh.Points()[k+1]
        if sliding_inner_reduced_mask.vec.FV()[k] > 0.5:
            sliding_inner_reduced_points.append(p)
            sliding_inner_reduced_points_idces.append(k)

        if sliding_inner_mask.vec.FV()[k] > 0.5:
            sliding_inner_points.append(p)
            sliding_inner_points_idces.append(k)
    ...
    a = (nsliding,2)
    idces_array = np.zeros(a, dtype=int)
    cumulative_d = 0

    for i in range(len(sliding_inner_reduced_points)):
        p = sliding_inner_reduced_points[i]
        p_idces = sliding_inner_reduced_points_idces[i]
        q = sliding_inner_points[i]
        q_idces = sliding_inner_points_idces[i]
        d = sqrt((p[0]-q[0])**2 + (p[1]-q[1])**2)
        #print(sliding_inner_reduced_points[i], sliding_inner_points[i])

        p[0] = r4*cos((i-1)/(nsliding-1)*2*pi)
        p[1] = r4*sin((i-1)/(nsliding-1)*2*pi)
    ...
    mesh.ngmesh.Update()
    print("airgap points with cumulative_d = ", cumulative_d)
    print("idces_array.shape", idces_array.shape)
...
if (buildMotor==True):
    ...
    mesh.ngmesh.Save("Motor_Bosch_2d_torque.vol")
else:
    mesh = ngsolve.Mesh("Motor_Bosch_2d_torque.vol")
    #inner radius rotor
    r1 = 26.5*10**(-3);
    #outer radius rotor
    r2_original = 78.63225*10**(-3);
...
sliding_inner_reduced_mask = GridFunction(fes)
sliding_inner_mask = GridFunction(fes)
rotor_outer_mask = GridFunction(fes)
airgap_mask = GridFunction(fes)

sliding_inner_reduced_mask.Set(CoefficientFunction((1)), definedon = mesh.Boundaries("sliding_inner_reduced"))
sliding_inner_mask.Set(CoefficientFunction((1)), definedon = mesh.Boundaries("sliding_inner"))
